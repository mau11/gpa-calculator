      ...
      <div className="container col-10">
        <form onSubmit={this.handleSubmit}>
          ...
          <div key={i} className="form-group form-inline col-12">
            <input className="form-control col-sm-5" type="text" name="class" id={'class'+i} placeholder={'Course #'+ (i+1)} />
            <select className="form-control col-6 col-sm-2" name="grade" id={'grade'+i}>
              {Object.keys(this.state.grades).map( (grade, index) => (
                <option key={index}>{grade}</option>
              ))}
            </select>
            <input className="form-control col-6 col-sm-3" type="number" name="credits" id={'credits'+i} placeholder="Credits" min="0.5" max="999" step="0.5" required />
